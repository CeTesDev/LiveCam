...
void Model3DTab::read3DModelProperties(FX* sourceFilter, FxWidget3D* sourceModel, FxWidget3D* destinyModel,
...
void Model3DTab::setShader(size_t objectIndex, std::string name)
{
	...
	FxWidget3D* model = (FxWidget3D*)editorWindow->editableFilter->models[curModelIndex].get();
	auto &renderParamsData = editorWindow->data.modules[curModelIndex].models[0].renderParams[objectIndex];

	auto iter = std::find_if(GraphicsModel::DefaultShadersMap.begin(), GraphicsModel::DefaultShadersMap.end(),
		[name](const GraphicsModel::DefaultShadersMapElement &E) -> bool
		{
			return E.first == name;
		}
		);
	auto shaders = iter->second;
	int shaderIndex = std::distance(GraphicsModel::DefaultShadersMap.begin(), iter);
...
void Model3DTab::setOBJ(std::vector<std::string> &fileNames, bool quickLoading)
{
	...
	auto model = (FxWidget3D*)editorWindow->editableFilter->models[curModelIndex].get();
	...
	if (quickLoading)
	{
		GraphicsLoader::loadObjectsNames(fileNames, objectsNames);
		...
		GraphicsLoader::loadMTLnames(fileNames, model->objects);
	}
	else
	{
		model->reloadOBJ();
	...
	for (auto &name : objectsNames)
	{
		auto shaders = GraphicsModel::DefaultShadersMap[0].second;
...
void Model3DTab::setMTL(std::vector<std::string> &fileNames, bool quickLoading)
{
	...
	try
	{
		for (auto &file : fileNames)
		{
			auto addMTLs = GraphicsLoader::convertMTLtoPTree(file.empty() ? "./assets/materials/no_material/no_material.mtl" : file,
	...
	auto noMTL = GraphicsLoader::convertMTLtoPTree("./assets/materials/no_material/no_material.mtl",
	...
	int meshIndex = 0;
	for (GraphicsData &obj : editorWindow->editableFilter->models[curModelIndex]->objects)
	{		
...
void Model3DTab::on_shiftX_slider_valueChanged(int value)
{
	...
	((FxWidget3D*)editorWindow->editableFilter->models[curModelIndex].get())->modelShift[0] = value / 100.;
...
void Model3DTab::on_shiftY_slider_valueChanged(int value)
{
	...
	((FxWidget3D*)editorWindow->editableFilter->models[curModelIndex].get())->modelShift[1] = value / 100.;
...
void Model3DTab::on_shiftZ_slider_valueChanged(int value)
{
	...
	((FxWidget3D*)editorWindow->editableFilter->models[curModelIndex].get())->modelShift[2] = value / 100.;
...
void Model3DTab::on_scaleX_slider_valueChanged(int value)
{
	...
	((FxWidget3D*)editorWindow->editableFilter->models[curModelIndex].get())->modelScale = value / 100.;
...
void Model3DTab::on_shiftX_spinBox_valueChanged(double value)
{
	...
	((FxWidget3D*)editorWindow->editableFilter->models[curModelIndex].get())->modelShift[0] = value;
...
void Model3DTab::on_shiftY_spinBox_valueChanged(double value)
{
	...
	((FxWidget3D*)editorWindow->editableFilter->models[curModelIndex].get())->modelShift[1] = value;
...
void Model3DTab::on_shiftZ_spinBox_valueChanged(double value)
{
	...
	((FxWidget3D*)editorWindow->editableFilter->models[curModelIndex].get())->modelShift[2] = value;
...
void Model3DTab::on_scaleX_spinBox_valueChanged(double value)
{
	...
	((FxWidget3D*)editorWindow->editableFilter->models[curModelIndex].get())->modelScale = value;
