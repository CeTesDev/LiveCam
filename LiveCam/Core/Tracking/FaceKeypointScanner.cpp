#include "FaceKeypointScanner.h"

#include <Common/CommonClasses.h>

FaceKeypointScanner::FaceKeypointScanner(bool use3D)
{
	this->use3D = use3D;
...
FaceKeypointScanner::~FaceKeypointScanner()
{
}

void FaceKeypointScanner::scale(double newscale)
{
	activescale = newscale;
}

cv::Mat FaceKeypointScanner::preProcess(cv::Mat frame)
{
	if (frame.channels() == 3)
	{
...
cv::Mat FaceKeypointScanner::preProcessForOneFace(cv::Mat frame, cv::Rect rect)
{
...
void FaceKeypointScanner::scan(TargetHoldingStruct &target, std::array<cv::Point2f, TARGET_DETAIL_MODIFIER>& landmarks)
{
	...
	{
		landmarks[i].x = int(featureBlob.at<float>(0, 2 * i) * newRect.width + newRect.x);
		landmarks[i].y = int(featureBlob.at<float>(0, 2 * i + 1) * newRect.height + newRect.y);
	}
